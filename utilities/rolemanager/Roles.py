...
OPERATIONAL_ROLES = ",".join(
    [x for x in ROLES if x not in [SUPER_USER_ROLE, ADMIN_ROLE, PM_ROLE]]
)
MANAGER_USER_ROLES = ",".join(
    [x for x in ROLES if x not in [SUPER_USER_ROLE, ADMIN_ROLE]]
)
...
def normalize(*args: str) -> str:
    ...
    for arg in args:
        ...
        roles.update(
            [x for x in [x.strip() for x in arg.upper().split(",")] if x and x in ROLES]
        )
