...
export class AnalysisController {
	...
	async getAnalysis(@Param("survey_id") surveyId: number) {
	...
	async getAnalysedMessages(
		...
		const result = await UserFeedbackMessage.createQueryBuilder("uf_messages")
			.where("uf_messages.programid = :program", {
				program: survey.project_code,
			})
			.andWhere("uf_messages.deploymentnumber = :deployment", { deployment })
			.andWhere("uf_messages.language = :language", { language })
			.andWhereExists(
				Analysis.createQueryBuilder("uf_analysis").where(
					"uf_analysis.message_uuid = uf_messages.message_uuid",
				),
			)
			...
			.leftJoinAndMapOne(
				...
				Recipient,
				"recipient",
				"recipient.recipientid = uf_messages.recipientid",
			)
			.leftJoinAndMapOne(
				"uf_messages.content_metadata",
				ContentMetadata,
				"content_metadata",
				"content_metadata.contentid = uf_messages.relation",
			)
			.getMany();
	...
	async deleteSubmission(
		...
		for (const result of results) {
			...
			const choices = await AnalysisChoice.find({
				where: { analysis_id: result.id },
			});
	...
	async saveOrCreate(
		...
		return ApiResponse.Success({
			data: await this.analysisService.createOrUpdate(surveyId, dto),
		});
	}
	...
	@Get(":survey_id/report")
	async report(
		@Param("survey_id") survey_id: number,
		@Query("message_id") message_id: string | undefined,
		@Query("language") language: string,
		@Query("deployment") deployment: string,
	) {
		return ApiResponse.Success({
			data: await this.analysisService.generateReport({
				survey_id,
				message_id,
				language,
				deployment,
