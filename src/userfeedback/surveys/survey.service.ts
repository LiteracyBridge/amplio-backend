...
export class SurveyService {
  ...
  async saveQuestions(dto: QuestionsDto, surveyId: number) {
    ...
    for (let i = 0; i < dto.sections.length; i++) {
      ...
      if ((section_dto.is_deleted ?? false) && !Number.isInteger(section_dto.id)) {
        await SurveySection.findOne({ where: { id: section_dto.id } })
          .then((section) => section?.softRemove())
        continue;
      }

      section_dto.new_id = (await this.createSection({ id: section_dto.id, name: section_dto.name }, surveyId)).id
    ...
    for (let i = 0; i < dto.questions.length; i++) {
      ...
      if ((question_dto.is_deleted ?? false) && !Number.isInteger(question_dto.id)) {
        await Question.findOne({ where: { id: question_dto.id } }).then((question) => question?.softRemove())
        continue;
      }
      ...
      question.question_label = question_dto.question_label ?? 'Untitled Question'
      ...
      if (parent_id != null) {
        ...
        if (_parent != null) {
          ...
          if (_found != null) {
            question_dto.parent_id = await this.createQuestion(surveyId, _found).then((q) => q.id)
          } else {
            throw new NotFoundException(`Parent question of question ${question_dto.id} cannot be found!`)
          }
        }
      }
      ...
      if (sectionId != null) {
        const _section = dto.sections.find(s => s.id === sectionId || s.new_id === sectionId)
        if (_section != null) {
          question_dto.section_id = await SurveySection.findOne({ where: { id: _section.new_id || _section.id } }).then((s) => s.id)
  ...
  private async createQuestion(survey_id: number, dto: Record<string, any>) {
    ...
    if (dto.id != null) {
      question = (await Question.findOne({ where: { id: dto.id } }))
  ...
  async _create_or_update_choices(opts: {
    ...
    for (let i = 0; i < dto.length; i++) {
      ...
      const choice = (await Choice.findOne({ where: { choice_id: option_id } })) ?? new Choice()
  ...
  private async createSection(dto: { id?: number, name?: string }, surveyId: number) {
    const sectionId = dto.id
    let section = new SurveySection()

    if (Number.isInteger(sectionId)) {
      section = (await SurveySection.findOne({ where: { id: sectionId } })) ?? new SurveySection()
    }
