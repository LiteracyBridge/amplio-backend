...
class StorePathAction(argparse.Action):
    ...
    def _expand(v: str):
        ...
        return v and Path(
            expanduser(v)
        )  # 'None' if v is None, otherwise Path(expanduser(v))

    def __init__(self, option_strings, dest, default: Optional[str] = None, **kwargs):
        if "glob" in kwargs:
        ...
        super(StorePathAction, self).__init__(
            option_strings,
            dest,
            default=self._expand(default),  # type: ignore
            **kwargs
        )

    def __call__(self, parser, namespace, values: Any, option_string=None):
        if self._glob and not isinstance(values, list):
            raise TypeError(
        ...
        if not isinstance(values, list):
            ...
            result = []
            for value in values:
                path: Path = self._expand(value)  # type: ignore
                path_str = str(path)
