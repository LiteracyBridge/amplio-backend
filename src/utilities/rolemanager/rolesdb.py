...
_ORGANIZATIONS_TABLE_CREATE_ARGS = {
    ...
    "AttributeDefinitions": [
        {"AttributeName": ORGS_ORGANIZATION_FIELD, "AttributeType": "S"}
    ],
...
_PROGRAMS_TABLE_CREATE_ARGS = {
    ...
    "AttributeDefinitions": [
        {"AttributeName": PROGRAMS_PROGRAM_FIELD, "AttributeType": "S"}
    ],
...
class RolesDb:
    def __init__(self, **kwargs):
        ...
        if False:
            ...
            self._dynamodb_client = boto3.client(
                "dynamodb", endpoint_url=config.dynamodb_url, region_name="us-west-2"
            )
            ...
            self._dynamodb_client = boto3.client(
                "dynamodb"
            )  # specify amazon service to be used
    ...
    def _delete_tables(self):
        ...
        end_time = time.time()
        print(
            "Deleted {} tables in {:.2g} seconds".format(
                num_updated, end_time - start_time
            )
        )
    ...
    def _create_tables(self):
        ...
        end_time = time.time()
        print(
            "Created {} tables in {:.2g} seconds".format(
                num_updated, end_time - start_time
            )
        )
    ...
    def get_organization_items(self):
        ...
        if time.time() > self._organizations_items_cache_expiry:
            ...
            self._organizations_items_cache = {
                x[ORGS_ORGANIZATION_FIELD]: x
                for x in self._organizations_table.scan()["Items"]
            }
            self._organizations_items_cache_expiry = (
                time.time() + _CACHE_TIMEOUT_SECONDS
            )
    ...
    def get_program_items(self):
        ...
        if time.time() > self._programs_items_cache_expiry:
            ...
            self._programs_items_cache = {
                x[PROGRAMS_PROGRAM_FIELD]: x
                for x in self._programs_table.scan()["Items"]
            }
