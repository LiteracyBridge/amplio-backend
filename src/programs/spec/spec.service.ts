...
export class ProgramSpecService {
  ...
  private async createExcel(project: Project, isExcel: boolean = true) {
    ...
    generalSheet.columns = Object.keys(headers.general).map(k => ({ header: isExcel ? headers.general[k] : k, key: k }));
    generalSheet.addRow({
      program_id: project.code, country: project.general.country, region: project.general.region,
      languages: project.languages.map(l => l.code), deployments_count: project.deployments.length,
      deployments_length: project.general.deployments_length,
      deployments_first: project.general.deployments_first,
      listening_models: project.general.listening_models, feedback_frequency: project.general.feedback_frequency,
      sustainable_development_goals: project.general.sustainable_development_goals, direct_beneficiaries_map: project.general.direct_beneficiaries_map,
      direct_beneficiaries_additional_map: project.general.direct_beneficiaries_additional_map,
      affiliate: project.general.affiliate, partner: project.general.partner
    });

    // Create deployments sheet
    const deploymentSheet = workbook.addWorksheet('Deployments');
    deploymentSheet.columns = Object.keys(headers.deployment).map(k => ({ header: isExcel ? headers.deployment[k] : k, key: k }));
    ...
    langSheet.columns = Object.keys(headers.languages).map(k => ({ header: isExcel ? headers.languages[k] : k, key: k }));
    ...
    contentSheet.columns = Object.keys(headers.content).map(k => ({ header: isExcel ? headers.content[k] : k, key: k }));
    for (const d of project.deployments) {
    ...
    recipientSheet.columns = Object.keys(headers.recipient).map(k => ({ header: isExcel ? headers.recipient[k] : k, key: k }));
      ...
      if (opts.format === "csv") {
        for (const k in names) {
          await client.send(new PutObjectCommand({
            ...
            Body: Buffer.from(await opts.xlsx.csv.writeBuffer({ sheetName: k })),
            Metadata: metadata
          }))
        }
      }
    } catch (error) {
      throw new InternalServerErrorException("Failed to upload excel file to S3")
    }
