...
export class AcmCheckoutService {
  ...
  private async report(dto: AcmCheckoutDto) {
    ...
    if (dto.recipient != null) {
      if (Array.isArray(dto.recipient)) {
        for (const r of dto.recipient) {
          if (!isEmail(r)) {
            throw new BadRequestException(`'${r}' recipient must be a valid email`)
          }
        }
        recipients.push(...dto.recipient)
      } else {
        if (!isEmail(dto.recipient)) {
          throw new BadRequestException(`'${dto.recipient}' recipient must be a valid email`)
        }
        recipients.push(dto.recipient)
      }
    } else {
      recipients.push(dto.to ?? appConfig().emails.support)
    }
