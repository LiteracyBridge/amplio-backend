...
def create_tables():
    ...
    end_time = time.time()
    print(
        "Created {} tables in {:.2g} seconds".format(num_updated, end_time - start_time)
    )
...
def load_tbloaderids_table(data=None):
    ...
    if data is None:
        data = [
            (1, "toffic@literacybridge.org", 966),
    ...
    end_time = time.time()
    print(
        "Added {} organizations in {:.2g} seconds".format(
...
def merge():
    ...
    for tbid in sorted(existing_tbids.keys()):
        print(
            '({}, "{}", {}),'.format(
                tbid, existing_tbids[tbid][0], existing_tbids[tbid][1]
            )
        )
    print("]")
...
def allocate_tbid_item(claims):
    ...
    new_item = {
        TBL_ID: new_max_id,
        TBL_HEX_ID: "{:04x}".format(new_max_id),
        TBL_EMAIL: email,
        TBL_RESERVE: 1,
    }
...
# @router.get("/reserve")
# def lambda_handler(n: str, context):
#     global tbloadersid_table
#     start = time.time_ns()
#     if tbloadersid_table is None:
#         open_tables()
...
#     parts = [
#         x
#         for x in event.get("pathParameters", {}).get("proxy", "reserve").split("/")
#         if x != "data"
#     ]
#     action = parts[0]
