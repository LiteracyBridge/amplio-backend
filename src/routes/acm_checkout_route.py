...
async def handle_request(request: Request):
    ...
    try:
        if len(body.keys()) > 0 or len(query.keys()) > 0:
            ...
            api_version = body.get("api")
            if not api_version:
                return v0_handler(body)
            elif api_version == "1":
                return V1Handler(body, query, user).handle_event()
...
class V1Handler:
    def __init__(self, body, query: QueryParams, user: User):
        self.event = body
        ...
        if query:
            ...
            self._program = body.get("db") or body.get("program")

        self._name = query.get("name") or user.email or body.get("name")
        self._contact = query.get("phone_number") or body.get("contact")
        self._version = query.get("version") or body.get("version")
        self._computername = query.get("computername") or body.get("computername")
        self._key = query.get("key") or body.get("key")
        self._filename = query.get("filename")
        self._comment = query.get("comment") or body.get("comment")

        print(
            f"Arguments: action:{self._action}, program:{self._program}, name:{self._name}, contact:{self._contact}"
        )
    ...
    def handle_event(self):
        ...
        return {STATUS: STATUS_DENIED, "response": "Unknown action requested"}
    ...
    def make_return(self, status, **kwargs):
        ...
        try:
            if self._checkout_record:
                retval["state"] = {
                    k: v
                    for k, v in self._checkout_record.items()
    ...
    def db_state(self):
        ...
        status = (
            "checkedout"
            if self._checkout_record.get("acm_state") == CHECKED_OUT
            else "available"
        )
        return status
    ...
    def checkout(self):
        ...
        version = (
            self._version
        )  # current ACM version in format: r YY MM DD n (e.g. r1606221)
        ...
        new_key = str(
            randint(0, 10000000)
        )  # generate new check-out key to use when checking ACM back in
    ...
    def checkin(self):
        ...
        checkout_key = (
            self._key
        )  # key must match check-key assigned at ACM check-out for check-in

        if (
            self._checkout_record.get("acm_state") != CHECKED_OUT
            or self._checkout_record.get("now_out_key") != checkout_key
        ):
            return self.make_return(STATUS_DENIED, error="Not checked out by user")

        filename = (
            self._filename
        )  # tracks number of times ACM has been checked out, format db##.zip (e.g. db12.zip)
        version = (
            self._version
        )  # current ACM version in format: r YY MM DD n (e.g. r1606221)
        ...
        update_expr = (
            build_remove_now_out(self._checkout_record)
            + "\
                    SET acm_state = :s, \
        ...
        condition_expr = (
            "now_out_key = :k"  # only check-in if the user has THIS check-out
        )
    ...
    def discard(self):
        ...
        def condition_handler(err):
            ...
            """
            if (
                self._checkout_record.get("acm_state") == CHECKED_IN
                or self._checkout_record.get("now_out_key") != checkout_key
            ):
        ...
        expr_values = {":s": CHECKED_IN, ":k": checkout_key}
    ...
    def reset(self):
        ...
        update_expr = (
            build_remove_now_out(self._checkout_record)
            + "SET acm_state = :s, last_in_file_name = :f, \
...
def v0_handler(event):
    try:
        action = event.get(
            "action"
        ).lower()  # 'checkIn', 'revokeCheckOut', 'checkOut', 'statusCheck', 'discard'
        if action == "report":
            return send_report(event)

        acm_name = event.get(
            "db"
        )  # name of ACM (e.g. 'ACM-FB-2013-01') - primary key of dynamoDB table
...
def check_in(event, acm):
    ...
    json_resp = {
        "success": {
            "response": "SUCCESS. Checked in by " + str(user_name),
            "data": STATUS_OK,
            STATUS: STATUS_OK,
        },
        "failure": {
            "response": "FAILED. Do not have permission to perform check-in",
            "data": STATUS_DENIED,
            STATUS: STATUS_DENIED,
        },
    }
...
def new_check_in(event, acm):
    ...
    condition_expr = (
        "attribute_not_exists(last_in_name)"  # only perform if no check-in entry exists
    )
    expr_values = {
        ":s": CHECKED_IN,
        ":f": file_name,
...
def discard(event, acm):
    ...
    condition_expr = "now_out_key = :k and now_out_name = :n"  # only perform check-in if THIS user has THIS check-out
    expr_values = {":s": CHECKED_IN, ":k": check_key, ":n": user_name}

    # JSON response parameters
    json_resp = {
        "success": {
            "response": "SUCCESS. Discarded check-out info",
            "data": STATUS_OK,
            STATUS: STATUS_OK,
        },
        "failure": {
            "response": "FAILED. Do not have permission to perform discard",
            "data": STATUS_DENIED,
            STATUS: STATUS_DENIED,
        },
    }
...
def check_out(event, acm):
    ...
    update_expr = "SET acm_state = :s, now_out_name = :n, now_out_contact = :c, now_out_key = :k, \
                            ...
                            now_out_computername = :m"
    condition_expr = (
        "attribute_not_exists(now_out_name)"  # check for unexpected check-out entry
    )
    expr_values = {
        ":s": CHECKED_OUT,
        ":n": user_name,
    ...
    json_resp = {
        "success": {
            "response": "SUCCESS. Checked out to " + str(user_name),
            "data": {
                "1": "key=" + new_key,
        ...
        "failure": {
            "response": "Your transaction was intercepted.",
            "data": STATUS_DENIED,
            STATUS: STATUS_DENIED,
        },
...
def status_check(acm):
    ...
    if acm.get("acm_state") == CHECKED_OUT:
        return {
            ...
            "data": {
                "1": "possessor=" + str(acm.get("now_out_name")),
                "2": "filename=" + str(acm.get("last_in_file_name")),
    ...
    return {
        ...
        "data": {
            "1": "filename=" + str(acm.get("last_in_file_name")),
...
def logger(event, response, action_name=None):
    ...
    action = str(
        event.get("action")
    )  # 'checkIn', 'revokeCheckOut', 'checkOut', 'statusCheck', 'discard'
...
def send_report(event):
    ...
    recipient = (
        ...
        or "ictnotifications@literacybridge.org"
    )
    recipients = recipient if isinstance(recipient, list) else [recipient]
...
def send_ses(fromaddr, subject, body_text, recipients, html=False):
    ...
    response = client.send_email(
        Source=fromaddr, Destination={"ToAddresses": recipients}, Message=message
    )
