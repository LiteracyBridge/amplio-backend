...
class Exporter:
    def __init__(self, program_id: str, db: Session, program_spec: Spec.Program = None):
    ...
    def read_from_database(self) -> Spec.Program:
        ...
        for row in result:
            self.program_spec.add_content(dict(row._mapping))

        result = query_table(Spec.recipient_sql_2_csv.keys(), "recipients")
        for row in result:
            self.program_spec.add_recipient(dict(row._mapping))
        ...
        result = query_table(
            ...
            "deployments",
            group_by=group_by,
            order_by="ORDER BY deploymentnumber",
        )
        for row in result:
            self.program_spec.add_deployment(dict(row._mapping))
    ...
    def do_save(
        ...
        def publish_csv(art: str) -> bool:
            ...
            try:
                ...
                result = False
                errors.append(
                    f"Could not convert {art}  for {self.program_id} to .csv: {str(ex)}."
                )
                return result
            ...
            if path is not None:
                try:
                    ...
                    result = False
                    errors.append(
                        f"Could not write {names[art]} for {self.program_id} as file: {str(ex)}"
                    )
            # Write to S3 if desirec.
            if bucket is not None:
                try:
                    s3_key = f"{self.program_id}/{names[art]}"
                    s3_put_result = s3.put_object(
                        Body=csv_str, Bucket=bucket, Key=s3_key, Metadata=metadata
                    )
                    if (
                        s3_put_result.get("ResponseMetadata", {}).get("HTTPStatusCode")
                        != 200
                    ):
                        result = False
                        errors.append(
        ...
        exporter: ExportProcessor.ExportProcessor = ExportProcessor.ExportProcessor(
            self.program_spec
        )
            ...
            if bucket is not None:
                ...
                try:
                    put_result = s3.put_object(
                        Body=data, Bucket=bucket, Key=key, Metadata=metadata
                    )
                    if (
                        put_result.get("ResponseMetadata", {}).get("HTTPStatusCode")
                        != 200
                    ):
                        errors.append(
    ...
    def save_unpublished(
        self, bucket: str, metadata: Dict[str, str] = None
    ) -> Tuple[bool, List[str]]:
        if not self._opened:
            raise Exception("Attempt to publish an un-opened program spec.")
        return self.do_save(
