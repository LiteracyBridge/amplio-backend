...
class ExportProcessor:
    ...
    def _add_sheet(
        wb: Workbook,
        sheet_name: str,
        values: List[Union[dict, dataclasses.dataclass]],
        header: List[str],
    ) -> Worksheet:
        """
    ...
    def _save_general(self, wb: Workbook) -> Worksheet:
        ...
        if self.program_spec._program is not None:
            ws = self._add_sheet(
                wb,
                sheet_name="General",
                values=[self.program_spec.program],
    ...
    def _get_deployments(
        self, for_csv: bool = False
    ) -> Tuple[List[str], List[Dict[str, str]]]:
        if for_csv:
            columns = list(Spec.deployment_sql_2_csv.keys()) + ["deployment"]
            values = [
                {k: v for k, v in dataclasses.asdict(x).items() if k in columns}
                for x in self.program_spec.deployments
            ]
        else:
            ...
            values = [
                {
                    ...
                    if k in Spec.deployment_sql_2_csv
                }
                for x in self.program_spec.deployments
            ]
    ...
    def _save_content(self, wb: Workbook) -> Worksheet:
        ...
        ws = self._add_sheet(
            wb, sheet_name="Content", values=self.program_spec.content, header=columns
        )
        formats = {
    ...
    def _save_recipients(self, wb: Workbook) -> Worksheet:
        ...
        ws = self._add_sheet(
            ...
            values=self.program_spec.recipients,
            header=columns,
        )
        self._auto_width(ws)
        return ws
    ...
    def get_csv(self, artifact: str) -> Optional[str]:
        ...
        if artifact == "general" and self.program_spec.program is not None:
            return self._get_csv(
                [self.program_spec.program], "general", Spec.general_sql_2_csv.keys()
            )
            ...
            return self._get_csv(
                self.program_spec.content, "content", Spec.content_sql_2_csv.keys()
            )
