...
class SpecCompare:
    def __init__(self, a: ProgramSpec, b: ProgramSpec):
        """Program spec loaded from the database."""
        self.a: ProgramSpec = a

        """Program spec uploaded by the user."""
    ...
    def diff_recipients(self) -> List[str]:
        ...
        def list_recip(r: Recipient, delta: str) -> None:
            ...
            results.append(
                f' Recipient {r.recipientid} "{fullname}" {delta}: {", ".join(values)}'
            )
        ...
        def diff_recip(a: Recipient, b: Recipient) -> None:
            ...
            fullname = "/".join(
                [a_d[n] for n in ["communityname", "groupname", "agent"] if a_d[n]]
            )
            results.append(
                f' Recipient {a.recipientid} "{fullname}" changed: {", ".join(changes)}'
            )
        ...
        added: List[str] = [
            recip_id for recip_id in b_dict.keys() if recip_id not in a_dict
        ]
        removed: List[str] = [
            recip_id for recip_id in a_dict.keys() if recip_id not in b_dict
        ]
        changed: List[str] = [
            recip_id
            for recip_id in a_dict.keys()
            if recip_id in b_dict and a_dict[recip_id] != b_dict[recip_id]
        ]
    ...
    def diff_content(self) -> List[str]:
        ...
        def list_message(message: Message, delta: str = "") -> None:
            msg = {
                k: _pr_val(v)
                for k, v in message.todict.items()
                if v and k not in ["position", "title"]
            }
            msg_str = ", ".join([f"{k}: {v}" for k, v in msg.items()])
            results.append(
                f'  Message @ {message.position} "{message.title}" {delta}: {msg_str}'
            )

        def list_playlist(
            playlist: Playlist, *, deploymentnumber: int, delta: str = ""
        ) -> None:
            results.append(
        ...
        def list_deployment(depl: Deployment, delta: str = "") -> None:
            ...
            for playlist in depl.playlists:
                list_playlist(
                    playlist, deploymentnumber=depl.deploymentnumber, delta=delta
                )
        ...
        def diff_message(a: Message, b: Message) -> None:
            ...
            changed: List[str] = [
                f
                for f in a_dict.keys()
                if a_dict[f] != b_dict[f] and (a_dict[f] or b_dict[f])
            ]
        ...
        def diff_playlist(a: Playlist, b: Playlist, deploymentnumber: int) -> None:
            ...
            messages_added: List[str] = [
                title for title in b_msgs.keys() if title not in a_msgs
            ]
            messages_removed: List[str] = [
                title for title in a_msgs.keys() if title not in b_msgs
            ]
            messages_changed: List[str] = [
                title
                for title in a_msgs.keys()
                if title in b_msgs and a_msgs[title] != b_msgs[title]
            ]
            # It changed, or we wouldn't be here.
            results.append(
                f' Playlist "{a.title}" in deployment # {deploymentnumber} changed: {", ".join(changes)}'
            )
        ...
        def diff_deployment(a: Deployment, b: Deployment) -> None:
            ...
            changed: List[str] = [
                f
                for f in a_map.keys()
                if a_map[f] != b_map[f] and (a_map[f] or b_map[f])
            ]
            changes = [f"{x}: {_pr_diff(a_map[x], b_map[x])}" for x in changed]
            ...
            playlists_added: List[str] = [
                title for title in b_plists.keys() if title not in a_plists
            ]
            playlists_removed: List[str] = [
                title for title in a_plists.keys() if title not in b_plists
            ]
            playlists_changed: List[str] = [
                title
                for title in a_plists.keys()
                if title in b_plists and a_plists[title] != b_plists[title]
            ]

            results.append(
                f'Deployment "{a.deploymentname or b.deploymentname or a.deployment}" deployment # {a.deploymentnumber} changed: {", ".join(changes)}'
            )
            for title in playlists_changed:
                diff_playlist(
                    a_plists[title],
                    b_plists[title],
                    deploymentnumber=a.deploymentnumber,
                )
            for title in playlists_added:
                list_playlist(
                    b_plists[title], delta="added", deploymentnumber=a.deploymentnumber
                )
        ...
        a_deployments: Dict[int, Deployment] = {
            d.deploymentnumber: d for d in self.a.deployments
        }
        b_deployments: Dict[int, Deployment] = {
            d.deploymentnumber: d for d in self.b.deployments
        }
        ...
        depls_added: List[int] = [
            n for n in b_deployments.keys() if n not in a_deployments
        ]
        depls_removed: List[int] = [
            n for n in a_deployments.keys() if n not in b_deployments
        ]
        depls_changed: List[int] = [
            n
            for n in a_deployments.keys()
            if n in b_deployments and a_deployments[n] != b_deployments[n]
        ]
