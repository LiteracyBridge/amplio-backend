...
class _SpecClass:
    ...
    def parse_date(x: Any, default=None) -> datetime.date:
        ...
        try:
            ...
            return datetime.datetime.strptime(
                datetime_pattern.search(x).group(), ymd_format
            ).date()
        except Exception:
            pass
        return default
    ...
    def parse_field(field_def, v):
        ...
        if field_def.type == str:
            ...
            field_def.type == list
            or field_def.type == dict
            or JSONABLE_TYPE_RE.match(str(field_def.type))
        ):
            return _SpecClass.parse_object(v)
        elif (
            field_def.type == datetime.date or field_def.type == datetime.datetime.date
        ):
    ...
    def parse(cls: dataclass, rec: Dict) -> Dict[str, Any]:
        ...
        mapped_rec = {
            in_name: rec[ext_name]
            for ext_name, in_name in cls.external_to_internal_map.items()
            if ext_name in rec
        }
        field_defs = {
            field_def.name: field_def
            for field_def in fields(cls)
            if field_def.metadata.get("import", True)
        }
        return {
            k: value_or_default(k, v) for k, v in mapped_rec.items() if k in field_defs
        }
    ...
    def format_field(field_def, value, target):
        formatter_given = (
            "formatter" in field_def.metadata
        )  # Is there an explicit formatter for this field (and possibly target)?
    ...
    def internal_to_csv_map(dc: dataclass) -> Dict[str, str]:
        ...
        return {
            field_def.name: alias(field_def)
            for field_def in fields(dc)
            if _SpecClass.should_export(field_def, "csv")
        }
    ...
    def external_to_internal_map(dataclass: dataclass) -> Dict[str, str]:
        ...
        dc_fields = [
            field_def
            for field_def in fields(dataclass)
            if field_def.metadata.get("import", True)
        ]
        return {
            alias: field_def.name
            for field_def in dc_fields
            for alias in aliases(field_def)
        }
    ...
    def csv_to_internal_map(cls: dataclass) -> Dict[str, str]:
        ...
        dc_fields = [
            field_def
            for field_def in fields(cls)
            if cls.should_export(field_def, "csv")
        ]
        return {
            alias: field_def.name
            for field_def in dc_fields
    ...
    def required_columns(cls: dataclass) -> List[str]:
        def required(field_def):
            # noinspection PyProtectedMember,PyUnresolvedReferences
            return (
                isinstance(field_def.default, dataclasses._MISSING_TYPE)
                and isinstance(field_def.default_factory, dataclasses._MISSING_TYPE)
            ) or field_def.metadata.get("required", False)
    ...
    def sql_columns(cls: dataclass) -> List[str]:
        return [
            field_def.name
            for field_def in fields(cls)
            if cls.should_export(field_def, "sql")
        ]
    ...
    def todict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            if _SpecClass.should_export(dcf, "csv")
        }
    ...
    def sql_row(self) -> Dict[str, Any]:
        ...
        for field_def in fields(self):
            ...
            if _SpecClass.should_export(field_def, "sql", value=value):
                result[field_def.name] = _SpecClass.format_field(
                    field_def, value, "sql"
                )
        return result
        ...
        for field_def in fields(self):
            ...
            if _SpecClass.should_export(field_def, "json", value=value):
                result[field_def.name] = (
                    str(value) if isinstance(value, datetime.date) else value
                )
        return result
...
class Recipient(_SpecClass):
    ...
    agent_gender: str = field(
        default=None, metadata={"parser": _SpecClass.parse_nullable_str}
    )
    direct_beneficiaries: int = None
    direct_beneficiaries_additional: dict = field(
        default_factory=dict, metadata={"as_json": True}
    )
    indirect_beneficiaries: int = None
    deployments: list[int] = field(
        default_factory=list,
        metadata={"parser": _SpecClass.parse_object, "as_depls": True},
    )
    recipientid: str = field(
        default=None, metadata={"alias": ["RecipientID", "RecipientId"]}
    )
    ...
    new_recipientid: bool = field(
        default=False, metadata={"export": [False], "import": False}
    )
    ...
    def __eq__(self, o: object) -> bool:
        ...
        op: Recipient = o
        eq: bool = (
            self.country == op.country
            and self.language == op.language
            and self.region == op.region
            and self.district == op.district
            and self.communityname == op.communityname
            and self.groupname == op.groupname
            and self.agent == op.agent
            and self.variant == op.variant
            and self.listening_model == op.listening_model
            and self.group_size == op.group_size
            and self.numhouseholds == op.numhouseholds
            and self.numtbs == op.numtbs
            and self.supportentity == op.supportentity
            and (
                self.agent_gender == op.agent_gender
                or not self.agent_gender
                and not op.agent_gender
            )
            and self.direct_beneficiaries == op.direct_beneficiaries
            and self.direct_beneficiaries_additional
            == op.direct_beneficiaries_additional
            and self.indirect_beneficiaries == op.indirect_beneficiaries
            and self.deployments == op.deployments
            and self.recipientid == op.recipientid
        )
        return eq
...
class Message(_SpecClass):
    ...
    playlist_title: str = field(
        default=None, metadata={"export": ["csv"], "alias": "Playlist Title"}
    )
    title: str = field(
        default=None,
        metadata={
            "alias": ["Message Title", "message_title"],
            "required": True,
            "csv": "message_title",
        },
    )
    ...
    languages: str = field(
        default=None,
        metadata={
            "alias": ["Language Code", "Languages", "languagecode"],
            "csv": "languagecode",
    ...
    format: str = None
    audience: str = None
    default_category_code: str = field(
        default=None,
        metadata={
            "alias": ["Default Category", "default_category"],
    ...
    sdg_goal: str = field(
        default=None,
        metadata={
            "alias": ["SDG Goals", "SDG Goal", "sdg_goals"],
            "parser": _SpecClass.parse_sdg,
            "csv": "sdg_goals",
        },
    )
    sdg_target: str = field(
        default=None,
        metadata={
            "alias": ["SDG Targets", "SDG Target", "sdg_targets"],
            "parser": _SpecClass.parse_sdg,
            "csv": "sdg_targets",
        },
    )
    ...
    parent: object = field(
        default=None, metadata={"export": [False], "import": False, "csv": None}
    )
    position: int = field(
        default=None, metadata={"export": [False], "alias": "message_pos", "csv": None}
    )
    ...
    def __eq__(self, o: object) -> bool:
        ...
        op: Message = o
        eq: bool = (
            self.deployment_num == op.deployment_num
            and self.playlist_title == op.playlist_title
            and self.title == op.title
            and self.key_points == op.key_points
            and self.languages == op.languages
            and (self.variant == op.variant or not self.variant and not op.variant)
            and (self.format == op.format or not self.format and not op.format)
            and (self.audience == op.audience or not self.audience and not op.audience)
            and self.default_category_code == op.default_category_code
            and self.sdg_goal == op.sdg_goal
            and self.sdg_target == op.sdg_target
            and self.position == op.position
        )
        return eq
...
class Playlist(_SpecClass):
    ...
    messages: List[Message] = field(
        default_factory=list, metadata={"export": [False], "import": False}
    )
    parent: object = field(default=None, metadata={"export": [False], "import": False})
    position: int = field(
        default=None, metadata={"export": [False], "alias": "playlist_pos"}
    )
...
class Deployment(_SpecClass):
    deploymentnumber: int = field(
        metadata={"alias": ["Deployment #", "deployment_num"]}
    )
    startdate: datetime.date = field(
        default=None, metadata={"alias": ["Start Date", "deployment_startdate"]}
    )
    enddate: datetime.date = field(
        default=None, metadata={"alias": ["End Date", "deployment_enddate"]}
    )
    deploymentname: str = field(
        default=None, metadata={"alias": ["Deployment Name", "deployment_name"]}
    )
    ...
    deployed: bool = field(
        default=None,
        metadata={"export": ["json", "value"], "import": "computed", "csv": None},
    )
    playlists: List[Playlist] = field(
        default_factory=list, metadata={"export": [False], "import": False, "csv": None}
    )
    ...
    def __eq__(self, o: object) -> bool:
        ...
        op: Deployment = o
        eq: bool = (
            self.deploymentnumber == op.deploymentnumber
            and self.startdate == op.startdate
            and self.enddate == op.enddate
            and self.deploymentname == op.deploymentname
            and self.playlists == op.playlists
        )
        return eq
...
class General(_SpecClass):
    program_id: str = field(
        metadata={"alias": ["Program ID", "program_id", "programid", "project"]}
    )
    name: str = None
    country: str = None
    ...
    listening_models: List[str] = field(
        default_factory=list, metadata={"as_json": True}
    )
    feedback_frequency: str = "one-quarter"
    sustainable_development_goals: list[int] = field(
        default_factory=list, metadata={"as_json": True}
    )
    ...
    direct_beneficiaries_additional_map: dict = field(
        default_factory=dict, metadata={"as_json": True}
    )
...
class ProgramSpec:
    ...
    def add_content(self, content: Dict) -> None:
        ...
        depls = [
            ...
            if d.deploymentnumber == depl_dict["deploymentnumber"]
        ]
    ...
    def infer_general_info(self):
        ...
        country = (
            Counter([recip.country for recip in self._recipients]).most_common(1)
            or "USA"
        )
    ...
    def write_to_db(
        self, engine: Engine = None, connection: Connection = None, **kwargs
    ) -> Tuple[bool, List[str]]:
    ...
    def create_from_json(
        programid: str, str_or_dict: Union[str, Dict], json_args=None
    ...
    @staticmethod
    def create_from_db(
        programid: str, engine: Engine = None, connection: Connection = None
    ) -> Tuple[object, List[str]]:
    ...
    @staticmethod
    def create_from_s3(
        programid: str, bucket: str = None, artifact: str = None, versionid: str = None
    ) -> Tuple[object, List[str]]:
        from ps import read_from_s3

        return read_from_s3(
            programid, bucket=bucket, artifact=artifact, versionid=versionid
        )
