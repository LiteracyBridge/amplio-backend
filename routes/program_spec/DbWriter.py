...
def _rows_differ(
    sql_row, ps_row, *, required_columns: List[str], ignored_columns: List[str], diffs
) -> bool:
    if sql_row != ps_row:
        ...
        diff_members = [
            ...
            if k not in ignored_columns and k in ps_row and v != ps_row[k]
        ]
...
class _DbWriter:
    ...
    def _save_name(self, programid: str, name: str):
        ...
        def get_dynamo_name() -> str:
            program_row = programs_table.get_item(Key={"program": programid}).get(
                "Item", {}
            )
        ...
        if dynamo_name != name:
            ...
            command = text(
                f"UPDATE projects SET project = :name WHERE projectcode = :program_id;"
            )
            result = self._connection.execute(command, values)
    ...
    def _merge_recipients(self):
        def differ(sql_row, ps_row: Recipient) -> bool:
            return _rows_differ(
                dict(sql_row),
                ps_row.todict,
                required_columns=Recipient.required_columns,
            ...
            if r.new_recipientid:
                ...
                sr0 = {
                    **dataclasses.asdict(r),
                    "project": self._program_spec.program_id,
        ...
        result = self._connection.execute(command, values)
        print(
            f"{result.rowcount} existing recipients for {self._program_spec.program_id}."
        )
        duplicate_additions = []
            ...
            if row_id in ps_recips:
                if differ(sql_row, ps_recips[row_id]):
                    ...
                    updates.append(
                        {
                            "project": self._program_spec.program_id,
                            **ps_recips[row_id].sql_row,
                        }
                    )
            # If any addition matches the sql row, drop that addition
            sql_dict = dict(
                sql_row
            )  # sqlalchemy row.get() throws instead of returning None when item not found.
            for addition in additions:
                duplicate = all(
                    (
                        sql_dict.get(col) == addition.get(col)
                        for col in [
                            "country",
                            "region",
                            "district",
        ...
        additions = [
            x for x in additions if x.get("recipientid") not in duplicate_additions
        ]
        ...
        if len(additions) > 0:
            ...
            print(
                f"{result_a.rowcount} recipients records added for {self._program_spec.program_id}."
            )
        ...
        if len(updates) > 0:
            ...
            print(
                f"{result_u.rowcount} recipients records updated for {self._program_spec.program_id}."
            )
    ...
    def _save_deployments_and_get_ids(self):
        def differ(sql_row, ps_row: Deployment) -> bool:
            ...
            return _rows_differ(
                db_dict,
                ps_dict,
                required_columns=Deployment.required_columns,
        ...
        ps_deployments: Dict[int, Deployment] = {
            d.deploymentnumber: d for d in self._program_spec.deployments
        }
        updates = []
        additions = []
        depl: Deployment
        for num, depl in ps_deployments.items():
            ...
            if num not in db_deployments:
                deployment = (
                    depl.deployment
                    or f"{self._program_spec.program_id}-{depl.startdate.year % 100}-{num}"
                )
                additions.append(
                    {
                        "program_id": self._program_spec.program_id,
        ...
        if len(updates) > 0:
            ...
            result_u = self._connection.execute(command_u, updates)
            print(
                f"{result_u.rowcount} deployment records updated for {self._program_spec.program_id}."
            )

    def _save_playlist(
        self, playlist: Playlist, deployment_id: int, deploymentnumber: int
    ) -> int:
        """
    ...
    def _save_message(self, message: Message, playlist_id: int) -> None:
        ...
        sdg_target = (
            message.sdg_target[message.sdg_target.find(".") + 1 :]
            if message.sdg_target and "." in message.sdg_target
            else message.sdg_target
        )

        # This would be better as a string column of the category name, because user may need to disambiguate.
        default_category_code = self._category_code_converter(
            message.default_category_code
        )
        ...
        values = {
            ...
            "sdg_target_id": message.sdg_target,
