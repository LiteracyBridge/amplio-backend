...
content_required_fields: List[str] = [
    "deployment_num",
    "playlist_title",
    "message_title",
]
...
if set(
    [
        x.name
        for x in fields(Content)
        if isinstance(x.default, dataclasses._MISSING_TYPE)
    ]
) != set(content_required_fields):
    raise Exception(
        'Fields from "Content" without defaults don\'t match "content_required_fields".'
    )
...
class Recipient:
    ...
    def __eq__(self, o: object) -> bool:
        ...
        eq: bool = (
            ...
            and self.agent_gender == op.agent_gender
...
if set(
    [
        x.name
        for x in fields(Recipient)
        if isinstance(x.default, dataclasses._MISSING_TYPE)
    ]
) != set(recipient_required_fields):
    raise Exception(
        'Fields from "Recipient" without defaults don\'t match "recipient_required_fields".'
    )
...
if list(
    [x.name for x in fields(Deployment) if x.name not in deployment_non_csv_columns]
) != list(deployment_sql_2_csv.keys()):
    raise Exception('Fields from "Deployment" don\'t match "deployment_sql_2_csv".')
if set(
    [
        x.name
        for x in fields(Deployment)
        if isinstance(x.default, dataclasses._MISSING_TYPE)
    ]
) != set(deployment_required_fields):
    raise Exception(
        'Fields from "Deployment" without defaults don\'t match "deployment_required_fields".'
    )
...
if set(
    [
        x.name
        for x in fields(General)
        if isinstance(x.default, dataclasses._MISSING_TYPE)
    ]
) != set(general_required_fields):
    raise Exception(
        'Fields from "General" without defaults don\'t match "general_required_fields".'
    )
...
def _normalize_record(record: Dict, record_def: Dict):
    return {
        k: _normalize(k, v, record_def) for k, v in record.items() if k in record_def
    }
...
class Program:
    ...
    def make_deployment(self, deployment: Dict[str, Any]) -> Deployment:
        ...
        if not deplname and startdate:
            deplname = (
                f"{self.program_id}-{startdate.strftime('%y')}-{deploymentnumber}"
            )
    ...
    def __eq__(self, o: object) -> bool:
        ...
        op: Program = o
        return (
            self.program_id == op.program_id
            and self._deployments == op._deployments
            and self._content == op._content
            and self._recipients == op._recipients
            and self._program == op._program
        )
...
class DbDeployment:
    ...
    def __eq__(self, o: object) -> bool:
        ...
        eq: bool = (
            ...
            and self.enddate == op.enddate
            and self.deploymentname == op.deploymentname
            and self.db_playlists == op.db_playlists
        )
        return eq


def flat_content_to_hierarchy(
    program_spec: Program, category_code_converter=lambda x: x
) -> Dict[int, DbDeployment]:
    def unflatten_content(content: Content) -> None:
        ...
        db_deployment = _db_deployments.setdefault(
            deployment_num, DbDeployment(deploymentnumber=deployment_num)
        )
        # Retrieve or create Playlist.
        db_playlist: DbPlaylist = db_deployment.db_playlists.setdefault(
        ...
        db_message = DbMessage(
            position=len(db_playlist.db_messages) + 1,
    ...
    for d in program_spec.deployments:
        _db_deployments[d.deploymentnumber] = DbDeployment(
            d.deploymentnumber, d.startdate, d.enddate, d.deploymentname, d.deployed
        )
    ...
    _db_deployments = {
        num: _db_deployments[num] for num in sorted(list(_db_deployments.keys()))
    }
...
def progspec_to_json(program_spec: Program) -> List[Dict]:
    ...
    """
    deployments: List[Dict] = sorted(
        [
            dataclasses.asdict(x)
            for x in flat_content_to_hierarchy(program_spec).values()
        ],
        key=lambda d: d.get("deploymentnumber"),
    )
    for depl in deployments:
        playlists = sorted(
            [x for x in depl["db_playlists"].values()],
            key=lambda pl: pl.get("position"),
        )
...
def progspec_from_json(programid: str, progspec_data: Union[List, Dict]) -> Program:
    def content_from_hierarchy(deployment_num: int, pl: Dict) -> None:
        ...
        if playlist_title:
            ...
            messages = sorted(
                [msg for msg in pl.get("messages") if msg.get("title")],
                key=lambda m: m.get("position"),
            )
            for msg in messages:
    ...
    def deployment_from_hierarchy(depl: Dict) -> None:
        ...
        result.add_deployment(
            {k: v for k, v in depl.items() if k in deployment_sql_2_csv.keys()}
        )
