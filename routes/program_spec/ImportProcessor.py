...
import routes.program_spec.spec as Spec
import db
...
def _rows_differ(
    db_row, csv_row, *, required_columns: List[str], ignored_columns: List[str], diffs
) -> bool:
    ...
    if db_row != csv_row:
        ...
        diff_members = [
            ...
            if k not in ignored_columns and k in csv_row and v != csv_row[k]
        ]
...
class ImportProcessor:
    ...
    def _get_category_code(self, categoryname: str) -> Optional[str]:
        ...
        try:
            command = text(
                "SELECT categorycode FROM supportedcategories WHERE categoryname=:categoryname;"
            )
    ...
    def _infer_program_info(self):
        ...
        country = (
            self._program.recipients[0].country
            if len(self._program.recipients) > 0
            else "USA"
        )
        self._db_program = Spec.General(
            ...
            region=json.dumps(list(regions)),
    ...
    def _merge_recipients(self):
        def differ(db_row, csv_row: Spec.Recipient) -> bool:
            return _rows_differ(
                dict(db_row),
                csv_row,
                required_columns=Spec.recipient_required_fields,
                ignored_columns=deprecated_columns,
                diffs=diffs,
            )
            ...
            if row_id in csv_recips:
                if differ(db_row, csv_recips[row_id]):
                    updates.append(
                        {
                            "project": self.program_id,
                            **dataclasses.asdict(csv_recips[row_id]),
                        }
                    )
        ...
        if len(additions) > 0:
            ...
            print(
                f"{result_a.rowcount} recipients records added for {self.program_id}."
            )
        # Update the changed ones
        if len(updates) > 0:
            command_u = text(
                f'UPDATE recipients SET ({",".join(field_names)}) '
                f'= (:{",:".join(field_names)}) WHERE recipientid=:recipientid;'
            )
            result_u = self._connection.execute(command_u, updates)
            print(
                f"{result_u.rowcount} recipients records updated for {self.program_id}."
            )

    def _get_deployment_ids(self) -> List[int]:
        command = text(
            "SELECT deploymentnumber, id FROM deployments WHERE project=:program_id;"
        )
        result = self._connection.execute(command, {"program_id": self.program_id})
        db_deployments: Dict[int, int] = {
            int(r["deploymentnumber"]): r["id"] for r in [dict(row) for row in result]
        }
    ...
    def _ensure_deployments_and_get_ids(self):
        def differ(db_row, csv_row: Spec.Deployment) -> bool:
            ...
            """
            return _rows_differ(
                self._program.make_deployment(dict(db_row)),
                csv_row,
                required_columns=Spec.deployment_required_fields,
                ignored_columns=["deployment"],
                diffs=diffs,
            )
        ...
        csv_deployments: Dict[int, Spec.Deployment] = {
            d.deploymentnumber: d for d in self._program.deployments
        }
        updates = []
        additions = []
                ...
                additions.append(
                    {
                        ...
                        "startdate": depl.startdate,
                        "enddate": depl.enddate,
                    }
                )
            else:
        ...
        if len(additions) > 0:
            ...
            result_a = self._connection.execute(command_a, additions)
            print(
                f"{result_a.rowcount} deployment records added for {self.program_id}."
            )
        ...
        if len(updates) > 0:
            if self._use_deployment_ids:
                command_u = text(
                    "UPDATE deployments SET project=:program_id, deploymentname=:deploymentname, "
            ...
            result_u = self._connection.execute(command_u, updates)
            print(
                f"{result_u.rowcount} deployment records updated for {self.program_id}."
            )

    def _save_playlist(
        self, deployment_id: int, db_playlist: Spec.DbPlaylist, deploymentnumber: int
    ) -> int:
        """
        ...
        command = text(
            f'INSERT INTO playlists ({",".join(columns)}) VALUES (:{",:".join(columns)}  );'
        )
        values = {
            "program_id": self.program_id,
            "deployment_id": deployment_id,
            "position": db_playlist.position,
            "title": db_playlist.title,
            "audience": db_playlist.audience,
        ...
        if self._use_deployment_ids:
            command = text(
                "SELECT id FROM playlists WHERE program_id=:program_id AND "
    ...
    def _save_message(self, playlist_id: int, db_message: Spec.DbMessage) -> None:
        ...
        command = text(
            f'INSERT INTO messages ({",".join(columns)}) VALUES (:{",:".join(columns)}  );'
        )
        values = {
            "program_id": self.program_id,
            "playlist_id": playlist_id,
            "position": db_message.position,
            "title": db_message.title,
            "format": db_message.format,
            "default_category_code": db_message.default_category_code,
            "variant": db_message.variant,
            "sdg_goal_id": db_message.sdg_goal_id,
            "sdg_target": db_message.sdg_target,
            "sdg_target_id": db_message.sdg_target_id,
            "key_points": db_message.key_points,
            "audience": db_message.audience,
            "languages": db_message.languages,
        }
        ...
        if db_message.languages and self._use_message_languages:
            ...
            message_id = -1
            command = text(
                "SELECT id FROM messages WHERE program_id=:program_id AND playlist_id=:playlist_id "
    ...
    def _save_content(self) -> None:
        ...
        for deployment_num, db_deployment in self._db_deployments.items():
            ...
            for db_playlist in db_deployment.db_playlists.values():
                # Create the playlist
                playlist_id = self._save_playlist(
                    deployment_id,
    ...
    def _update_database(self, content_only: bool = False):
        self._use_deployment_ids = not db.table_has_column(
            "playlists", "deploymentnumber"
        )
        ...
        self._deployments_with_content = [
            n for n, depl in self._db_deployments.items() if len(depl.db_playlists) > 0
        ]
        ...
        print(
            f"{len(content)} messages after save for {self.program_id} (was {num_messages})."
        )
    ...
    def update_db_program_spec(
        ...
        self._db_deployments = Spec.flat_content_to_hierarchy(
            self._program, category_code_converter=self._get_category_code
        )
