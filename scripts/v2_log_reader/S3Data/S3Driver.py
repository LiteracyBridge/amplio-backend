...
class S3Driver:
    def __init__(self, prefix: str, args: argparse.Namespace):
        ...
        self._kwargs = (
            {k: v for k, v in args.__dict__.items() if isinstance(v, bool)}
            if args
            else {}
        )
    ...
    def find_objects(self) -> bool:
        self._s3_objects = [
            ...
            for x in list_objects(
                bucket=self._source_bucket, prefix=self._source_prefix
            )
        ]
    ...
    def process_objects(self):
        ...
        for s3_obj in self._s3_objects:
            if s3_obj["Key"][-1] != "/":  # ignore the s3 equivalent of directories.
                s3_importer = S3Importer(
                    self._source_bucket, s3_obj, self._now, **self._kwargs
                )
                if s3_importer.is_valid:
                    s3_importer.do_import()
                    summary = s3_importer.summary
                    self._details.setdefault(summary.programid, {}).setdefault(
                        summary.names, []
                    ).append(summary)
        ...
        for programid, names_list in self._details.items():
            program_summary = summaries.setdefault(
                programid,
                {
                    "names": set(),
                    "keys": {},
            ...
            for names, summary_list in names_list.items():
                ...
                for s in summary_list:
                    ...
                    if s.have_statistics:
                        ...
                        program_summary["disposition"][s.disposition] = (
                            program_summary["disposition"].get(s.disposition, 0) + 1
                        )

        for programid, program_summary in summaries.items():
            print(
                f'Summary for {programid}; processed {len(program_summary.get("keys"))} zip file(s), from {len(program_summary.get("names"))} TB-Loader(s).'
            )
            print(
                f'    {program_summary.get("collections", "no")} collection(s), {program_summary.get("deployments", "no")} deployment(s), '
            ...
            print(
                f'      disposition(s): {", ".join([f"{k}:{v}" for k,v in dispositions.items()])}'
            )
            ...
            print(
                line_breaker(
                    ...
                    [
                        zip_summary(x)
                        for x in sorted(program_summary["keys"].values(), key=rev_k)
                    ],
                )
            )
