...
def process_tb_collected_data(path: Path, num=0) -> None:
    ...
    tbd.save_operational_csvs(
        tbsdeployed="tbsdeployed.csv", tbscollected="tbscollected.csv"
    )
...
def process_v1_operation_data(dirs: list[Path]) -> int:
    ...
    for dir in dirs:
        ...
        if tbsdeployed.exists() and not args.force:
            ...
            print(
                f"Can not create {tbsdeployed.name} because file {deployments.name} does not exist."
            )
        else:
            if verbose >= 2:
                print(
                    f'{"Deleting and re-c" if tbsdeployed.exists() else "C"}reating {tbsdeployed.name}.'
                )
        ...
        if tbscollected.exists() and not args.force:
            ...
            missing = " and ".join(
                [x.name for x in [stats_collected_props, tbData] if not x.exists()]
            )
            print(
                f"Can not create {tbscollected.name} because files {missing} do not exist."
            )
        else:
            if verbose >= 2:
                print(
                    f'{"Deleting and re-c" if tbscollected.exists() else "C"}reating {tbscollected.name}.'
                )
...
def report():
    if _num_uf is None:
        ...
        print(
            f'{_num_files} zip files/directories processed {"" if _num_uf is None else details}.'
        )
...
def main():
    ...
    arg_parser = argparse.ArgumentParser(
        description="Read TB-2 log files and create statistics CSV files."
    )
    arg_parser.add_argument(
        "--playstatistics",
        "-ps",
        action=StorePathAction,
        help="Store playstatistics here.",
    )
    arg_parser.add_argument(
        "--verbose", "-v", action="count", default=0, help="More verbose output."
    )
    ...
    arg_parser.add_argument(
        ...
        "-n",
        action="store_true",
        help="Dry run, do not write to s3, do not insert to db.",
    )
    arg_parser.add_argument(
        "--no-uf",
        action="store_true",
        help="Do not publish uf files, nor update uf_messages table.",
    )
    arg_parser.add_argument(
        "--uf-only",
        action="store_true",
        help="Only publish uf files and update uf_messages table.",
    )

    arg_parser.add_argument(
        "--no-db", action="store_true", help="Do not update the database."
    )
    arg_parser.add_argument(
        "--no-s3", action="store_true", help="Do not publish anything to s3."
    )

    arg_parser.add_argument(
        "--no-archive",
        action="store_true",
        help="Do not move the timestamp.zip file (eg 202212119T111230.123Z.zip) to the archived-data.v2 area.",
    )
    arg_parser.add_argument(
        "--archive-only",
        action="store_true",
        help="Only move the timestamp.zip file to the archive area; do not update any database or other s3 area.",
    )
    ...
    arg_parser.add_argument(
        ...
        help="S3 bucket in which to find collected data.",
    )
    arg_parser.add_argument(
        "--s3",
        nargs="*",
        action="store",
        help="Prefix to objects. If the value starts with '/', it is considered an absolute path under the S3 bucket; "
    ...
    if (
        len(
            conflicts := [
                x for x in ["dry_run", "uf_only", "archive_only"] if bool_args.get(x)
            ]
        )
        > 1
    ):
