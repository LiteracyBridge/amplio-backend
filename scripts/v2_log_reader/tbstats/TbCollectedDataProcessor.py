...
DEPLOYMENTS_LOG_TO_TBSDEPLOYED_CSV = {
    "sn": "talkingbookid",
...
TBSDEPLOYED_CSV_TO_DEPLOYMENTS_LOG = {
    "talkingbookid": "sn",
    "recipientid": "recipientid",
...
TBDATA_LOG_TO_TBSCOLLECTED_CSV = {
    "in_sn": "talkingbookid",
    "in_recipientid": "recipientid",
    "timestamp": "collectedtimestamp",
    "in_project": "project",
    "in_deployment": "deployment",
    "in_package": "contentpackage",
    "in_firmware": "firmware",
    "location": "location",
    "latitude": "latitude",
    "longitude": "longitude",
    "username": "username",
    ...
    "action": "action",
    "in_testing": "testing",
    "in_deployment_uuid": "deployment_uuid",
    "stats_uuid": "collection_uuid",
}
TBSCOLLECTED_CSV_TO_TBDATA_LOG = {
    "talkingbookid": "in_sn",
    "recipientid": "in_recipientid",
    "collectedtimestamp": "timestamp",
    "project": "in_project",
    "deployment": "in_deployment",
    "contentpackage": "in_package",
    "firmware": "in_firmware",
    "location": "location",
    "latitude": "latitude",
    "longitude": "longitude",
    "username": "username",
    ...
    "action": "action",
    "testing": "in_testing",
    "deployment_uuid": "in_deployment_uuid",
...
STATS_COLLECTED_PROPERTIES_TO_TBSCOLLECTED_CSV = {
    "deployment_TALKINGBOOKID": "talkingbookid",
...
def _query_recipient_language(recipientid: str) -> str | None:
    """
...
class TbCollectedData:
    def __init__(
        self, zip_file_path: Path, verbose: int = 0, timestamp_str=None, **kwargs
    ):
        """
        ...
        if zip_file_path.is_dir():
            ...
            self._tb_collected_data_path: Path = Path(
                temp_path, zip_file_path.with_suffix("").name
            )
        ...
        stats_collected_properties_path = Path(
            self._tb_collected_data_path, "stats_collected.properties"
        )
        if not stats_collected_properties_path.exists():
            # try OperationalData subdirectory
            stats_collected_properties_path = Path(
                self._tb_collected_data_path,
                "OperationalData",
                "stats_collected.properties",
            )
        if stats_collected_properties_path.exists():
            self._stats_collected_properties_data: dict[str, str] = (
                load_properties_file(stats_collected_properties_path)
            )
    ...
    def _load_collected_talking_book_files(self):
        ...
        if not self._stats_collected_properties_data:
            ...
            self._stats_collected_properties_data: dict[str, str] = (
                load_properties_file(properties_path)
            )

        # Create the Statistics object, to accumulate info from the log files.
        self._play_statistics: Statistics = Statistics(
            self._stats_collected_properties_data
        )
        # Read the log files.
        ...
        if logs_path.is_dir():
            ...
            for log_path in log_paths:
                lr: LogFileReader = LogFileReader(
                    log_path,
                    self._packages_data,
                    latest_time,
            ...
            if num_errors > 0:
                print(
                    f"{num_errors} processing {self._zip_file_name} from {self._data_source_name}"
                )
            else:
                print(
                    f"Processed stats in {self._zip_file_name} from {self._data_source_name}"
                )
        else:
            print(
                f'No "log" directory in {self._zip_file_name} from {self._data_source_name}'
            )
    ...
    def has_statistics(self) -> bool:
        try:
            return (
                self._play_statistics
                and len(self._play_statistics._play_statistics) > 0
            )
        except Exception:
            pass
        return False
    ...
    def save_playstatistics_csv(self, playstatistics_file):
        ...
        :return: None
        """
        self._play_statistics.emit(
            self._stats_collected_properties_data, playstatistics_file
        )

    @property
    def playstatistics(self) -> List[Dict]:
        try:
            return self._play_statistics.get_playstatistics(
                self._stats_collected_properties_data
            )
        except:
            return []
    ...
    def deployment_num(self) -> int:
        try:
            return int(
                self._stats_collected_properties_data.get(
    ...
    def deployment_language(self):
        ...
        if not language:
            recipientid = self._stats_collected_properties_data.get(
                "deployment_RECIPIENTID"
            )
    ...
    def is_stats_only(self) -> bool:
        ...
        """
        try:
            return self._stats_collected_properties_data.get("ACTION", "").startswith(
                "stats"
            )
        except:
            return False
    ...
    def tbdata_log(self) -> dict:
        def is_valid_tb_data() -> bool:
            ...
            good = [
                x is not None and x != "UNKNOWN" and x != "-- TO BE ASSIGNED --"
                for x in values
            ]
            return all(good)
    ...
    def tbscollected(self) -> Optional[dict]:
        ...
        try:
            if tbdata_log := self.tbdata_log:
                ...
                data = {
                    c: (tbdata_log[map[c]] if map[c] in tbdata_log else None)
                    for c in cols
                }
                ...
                for (
                    stc_name,
                    tbc_name,
                ) in STATS_COLLECTED_PROPERTIES_TO_TBSCOLLECTED_CSV.items():
                    if stc_name in self._stats_collected_properties_data:
                        if not data.get(
                            tbc_name
                        ) and self._stats_collected_properties_data.get(stc_name):
                            if self._verbose >= 2:
                                print(
                                    f"Filling missing property, tbData[{tbc_name}] = {self._stats_collected_properties_data.get(stc_name)}"
                                )
                            data[tbc_name] = self._stats_collected_properties_data.get(
                                stc_name
                            )
            ...
            traceback.print_exc()
            print(
                f"Exception creating tbscollected.csv from {self._zip_file_name}: {ex}"
            )
            return None
        return data
    ...
    def tbsdeployed(self) -> Optional[dict]:
        ...
        try:
            if deployments_log := self.deployments_log:
                ...
                data = {
                    c: (deployments_log[map[c]] if map[c] in deployments_log else None)
                    for c in cols
                }
            ...
            traceback.print_exc()
            print(
                f"Exception creating tbsdeployed.csv from {self._zip_file_name}: {ex}"
            )
            return None
        return data
        ...
        if tbscollected_csv := kwargs.get("tbscollected"):
            if tsbcollected_data := self.tbscollected:
                do_save(
                    tbscollected_csv,
                    "tbscollected.csv",
                    headings=TBSCOLLECTED_CSV_COLUMNS,
                ...
                missing = []
                if not self._stats_collected_properties_data:
                    missing.append("stats_collected.properties file")
                if not self.tbdata_log:
                    missing.append(f"{self._tbdata_log_path.name} file")
                print(
                    f'Unable to find (or read) {" or ".join(missing)}; can not create {_nameof(tbscollected_csv)}'
                )

        if tbsdeployed_csv := kwargs.get("tbsdeployed"):
            if (
                self._stats_collected_properties_data
                and self._stats_collected_properties_data.get("ACTION", "").startswith(
                    "stats"
                )
            ):
                if self._verbose:
                ...
                if tsbdeployed_data := self.tbsdeployed:
                    do_save(
                        tbsdeployed_csv,
                        "tbsdeployed.csv",
                        headings=TBSDEPLOYED_CSV_COLUMNS,
                    ...
                    # Why not?
                    missing = []
                    if not self.self.deployments_log:
                        missing.append(f"{self._deployments_log_path.name} file")
                    print(
                        f'Unable to find (or read) {" or ".join(missing)}; can not create {_nameof(tbsdeployed_csv)}'
                    )
