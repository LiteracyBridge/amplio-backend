...
class PlayStatistic(Statistic):
    ...
    def played(self, duration_ms: int, played_ms: int):
        ...
        if played_ms > duration_ms - 2000:
            self.completions += 1
        elif played_ms > (duration_ms - 2000) * 0.75:
            self.three_quarters += 1
        elif played_ms > (duration_ms - 2000) * 0.5:
            self.half += 1
        elif played_ms > (duration_ms - 2000) * 0.25:
            self.one_quarter += 1
        elif played_ms > 2000:
            self.started += 1
    ...
    def emit_played_event(
        self, duration_ms: int, played_ms: int, properties: [str, dict], outf
    ):
        pass
    ...
    def as_dict(self, properties: dict[str, str]) -> dict:
        ...
        deployment_timestamp = Statistic.normalize_timestamp(
            properties.get("deployment_TIMESTAMP")
        )
