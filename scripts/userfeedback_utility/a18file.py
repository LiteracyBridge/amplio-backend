...
class MetadataReader:
    ...
    def parse(self) -> Dict[str, str]:
        ...
        if version != 1:
            raise ValueError(
                f"Unknown metadata version. Expected '1', but found '{version}'."
            )
...
class A18File:
        ...
        if self.has_sidecar or creating:
            try:
                ...
                if not self.property(MD_MESSAGE_UUID_TAG):  # includes 'metadata.' tag.
                    self.add_to_sidecar(
                        {MD_MESSAGE_UUID_TAG: str(self._compute_message_uuid())}
                    )

                # Ensure deployment number is in the sidecar. This is performed at most one time.
                if not self.property(DEPLOYMENT_NUMBER_TAG):
                    deployment_number = self._db_utils.query_deployment_number(
                        self.property(PROJECT_TAG), self.property(DEPLOYMENT_TAG)
                    )
                ...
                recipient_info = self._db_utils.query_recipient_info(
                    self.property(RECIPIENTID_TAG)
                )
                ...
                print(
                    f"Exception updating sidecar for '{str(self._file_path)}': {str(ex)}"
                )
        return False

    def create_sidecar(
        self, recipientid: str, programid: str, deploymentnumber: int, community: str
    ) -> bool:
        """
    ...
    def save_sidecar(
        ...
        extra_data: Union[None, Dict[str, str]] = None,
    ) -> Dict[str, str]:
        """
            ...
            if self._dry_run:
                ...
                with open(temp_path, "w") as properties_file:
                    for h in self._sidecar_header:
                        print(
                            h, file=properties_file, end="\x0d\x0a"
                        )  # microsoft's original sin
                    for k in sorted(to_write.keys()):
                        print(
                            f"{k}={to_write[k]}", file=properties_file, end="\x0d\x0a"
                        )
    ...
    def add_to_sidecar(self, data: Dict[str, str], tag: str = None) -> None:
        ...
        for k, v in data.items():
            tagged_key = f"{tag}{k}"
            if (
                tagged_key not in self._sidecar_data
                or self._sidecar_data[tagged_key] != v
            ):
                if self._verbose > 2:
    ...
    def export_audio(
        self, audio_format: str, output: Path = None, mk_dirs: bool = True
    ) -> Union[Path, None]:
        """
        ...
        tdp = Path(target_dir, ".")
        print(
            f"Target dir: {target_dir}, exists:{target_dir.exists()}, is_dir:{target_dir.is_dir()}"
        )
        ...
        if self._local_ffmpeg:
            ...
            tempfile_pathname: str = (
                f"{tmp_dir.name}/{source_name}.wav"  # ...tmp/foo.a18.wav
            )
            ...
            if ac_result.returncode != 0:
                if (
                    ...
                    in ac_result.stderr.decode("utf-8").lower()
                ):
