...
def create_program_record(
    ...
    """
    print(
        "Creating program record for {} in organization {}".format(
    ...
    if admin_email:
        ...
        # item["roles"] = {admin_email: ADMIN_ROLES} # FIXME: update with new user roles
...
def check_for_organization_record(organization: str, parent_organization: str) -> bool:
    ...
    if organization_row:
        ...
        parent_row = organization_table.get_item(
            Key={"organization": parent_organization}
        )
...
def create_organization_record(organization: str, parent_organization: str) -> bool:
    ...
    print(
        f"Creating organization record for {organization} with parent {parent_organization}",
        end="",
    )
    update_expr = "SET parent = :p"
    expr_values = {":p": parent_organization}
