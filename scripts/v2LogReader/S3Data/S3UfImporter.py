...
_UF_RELATION_PATTERN = re.compile(
    r"(?ix)uf_pkg(?P<pkg>\d+)_pl(?P<pl>\d+)_msg-?(?P<msg>\d+).*"
)
_UF_RECORDING_PATTERN = re.compile(
    r"(?ix)uf_pkg(?P<pkg>\d+)_pl(?P<pl>\d+)_msg-?(?P<msg>\d+)_(?P<no>\d+).*"
)
...
class S3UfImporter:
    def __init__(
        self, temp_dir_path: Path, tb_collected_data: TbCollectedData, **kwargs
    ):
        ...
        for zipinfo in zipfile.infolist():
            if m := _UF_FILE_PATTERN.match(zipinfo.filename):
                ...
                if m.group("suffix").lower() == ".properties":
                    ...
                    data = zipfile.read(zipinfo.filename)
                    self._surveys[m.group("name")] = decode_properties(
    ...
    def decrypt_and_convert(self, zipfile: ZipFile, suffixes, stem):
        ...
        if session_key:
            ...
            cipher = Cipher(algorithms.AES(session_key), modes.CBC(iv))  # type: ignore
    ...
    def convert_wav_to_mp3(self, raw_data: bytes, fn: str) -> bool:
        ...
        def fix_wav_length(data_to_fix: bytes) -> bytearray:
            """
        ...
        def duration(wav_bytes: bytearray) -> int:
            """
        ...
        def get_props():
            ...
            if m := _UF_RELATION_PATTERN.match(fn):
                ...
                if pkg := self._tb_collected_data.packages_data.get_package(
                    int(m.group("pkg"))
                ):
                    props = {
                        "PACKAGE_NAME": pkg.name,
                        "PACKAGE_NUM": int(m.group("pkg")),
        ...
        duration = int(duration(data) + 0.5)  # type: ignore
    ...
    def fill_userrecordings_metadata(self):
        ...
        for fn, properties in self._userrecordings_properties.items():
            if (uuid := properties.get("metadata.MESSAGE_UUID")) and (
                ...
                properties["USERNAME"] = collection_props.get(
                    "deployment_USEREMAIL", collection_props.get("deployment_USERNAME")
                )
                properties["TESTDEPLOYMENT"] = collection_props[
                    "deployment_TESTDEPLOYMENT"
                ]
                ...
                properties["metadata.LANGUAGE"] = (
                    self._tb_collected_data.deployment_language
                )
                if m := _UF_RELATION_PATTERN.match(fn):
                    message = (
                        self._tb_collected_data.packages_data.get_message_by_index(
                            int(m.group("pkg")), int(m.group("pl")), int(m.group("msg"))
                        )
                    )
                    if relation := message.filename if message else None:
                        properties["metadata.RELATION"] = (
                            Path(relation).with_suffix("").name
                        )
                ...
                uf_messages_row = {
                    _UF_PROPERTY_TO_UF_COLUMN[k]: properties.get(k)
                    for k in _UF_PROPERTY_TO_UF_COLUMN.keys()
                }
        ...
        with uf_messages_path.open("w") as uf_messages_file:
            ...
            for row in self._uf_messages_csv_rows:
                print(
                    ",".join(
                        [escape_csv(row[x]) for x in _UF_PROPERTY_TO_UF_COLUMN.values()]
                    ),
                    file=uf_messages_file,
                )
    ...
    def fill_survey_metadata(self):
        ...
        for fn, survey in self._surveys.items():
            ...
            for key, value in survey.items():
                ...
                updates["programid"] = collection_props[
                    "deployment_PROJECT"
                ]  # for convenience only; implicit in recipientid
    ...
    def recover_session_key(
        self, encoded_session_key, encoded_iv
        ...
        def get_private_key(
            programid: str, deployment_num: int
        ...
        if private_key := get_private_key(programid, deployment_num):
            ...
            session_key = private_key.decrypt(  # type: ignore
