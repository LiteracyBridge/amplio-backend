...
recipient_overrides = {
    ...
    "CARE": {"KPATUA NO": "KPATUA NO 1", "SONGO ANONGTAABA": "SONGO ANONGTAABA 1"},
}
...
def lookup_recipient(proj, directory):
    ...
    if directory not in recipient_map[proj]:
        if proj not in comm_warnings or directory not in comm_warnings[proj]:
            if proj not in comm_warnings:
                comm_warnings[proj] = {}
            comm_warnings[proj][directory] = True
            sys.stdout.write(
                "directory {} is not in map for {}.\n".format(directory, proj)
            )
        return None
...
def main():
    ...
    arg_parser = argparse.ArgumentParser(
        description="Extract deployments data", usage=usage
    )
    arg_parser.add_argument(
        "data", nargs="*", help="File name(s), contain(s) the key:value data."
    )
    arg_parser.add_argument(
        "--no-header", "--noheader", action="store_false", dest="header"
    )
    arg_parser.add_argument(
        "--columns",
        nargs="*",
        help="List of column names, in order, or @filename with list",
    )
    ...
    arg_parser.add_argument(
        ...
        default=False,
        dest="two_pass",
        help="Two pass operation; don't load all data into memory.",
    )
    arg_parser.add_argument(
        "--map", help="Optional csv file of project+community => recipientid."
    )
